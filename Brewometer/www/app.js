...
app.onConnect = function(peripheral) {
	...
	peripheral.brewometerData = {};
	peripheral.messageCount = 0;
	...
	ble.startNotification(peripheral.id, app.beanAppMessageServiceUUID, app.beanAppMessageCharacteristicUUID,
		function(data) {
			...
			app.parseBeanResponse(peripheral, data);
			console.log(data);
		},
		function(errorcode) {
			console.log('BLE startNotification error: ' + errorCode);
...
app.constructBeanMessage = function getBuffer(device, cmdBuffer, payloadBuffer) {
	...
	var crc16Buffer = app.ccitt(gstBuffer);

	console.log('crc16Buffer ' + app.hexStringFromUint8Array(crc16Buffer));

	//GATT contains sequence header, gstBuffer and crc166
	var gattBuffer = new Uint8Array(1 + gstBuffer.length + crc16Buffer.length);

  	var header = (((device.messageCount++ * 0x20) | 0x80) & 0xff);
...
app.setBeanLed = function(device, r, g, b) {
	...
	app.sendBeanAppMessage(device, 0x20, 0x01, payload);
}

app.requestBeanAccelerometer = function(device) {
	app.sendBeanAppMessage(device, 0x20, 0x10, new Uint8Array(0));
}
...
app.sendBeanAppMessage = function(device, major, minor, payload) {
	...
	var data = app.constructBeanMessage(device, cmdBuffer, payload);

	ble.writeWithoutResponse(device.id, app.beanAppMessageServiceUUID, app.beanAppMessageCharacteristicUUID, data.buffer, function() {
...
app.ui.displayDeviceList = function() {
	...
	for (var item in app.connectedDevices) {
		...
		if(device.pitch != null){
			listItem.innerHTML += device.pitch.toFixed(2) + "°";
		}
		listItem.innerHTML += '<br />';
		...
		// //accelerometer
		// device.requestBeanAccelerometerButton = document.createElement("button");
		// device.requestBeanAccelerometerButton.className = "red";
		// device.requestBeanAccelerometerButton.innerHTML = "Get reading  " + device.id;
		// device.requestBeanAccelerometerButton.id = "requestBeanAccelerometerButton " + device.id;
		// device.requestBeanAccelerometerButton.onclick = function() {
		// 	// payload = new Uint8Array(1);
		// 	// payload[0] = 0x0d;
		// 	// app.sendBeanAppMessage(device,0x00,0x00,payload);
		// 	console.log("requestBeanAccelerometerButton click " + this.id);
		// 	app.requestBeanAccelerometer(device);

		// };
		// listItem.appendChild(device.requestBeanAccelerometerButton);
		...
		// device.ledButton.id = "ledButton" + device.id;
		// device.ledButton.onclick = function() {
		// 	console.log("ledButton click " + this.id);
		// 	app.setBeanLed(device,0x00,0x00,0x00);
		...
		device.disconnectButton.id = "disconnectButton "+device.id;
...
app.parseBeanResponse = function(peripheral, data) {

	dataArray = new Uint8Array(data);

	console.log(app.hexStringFromUint8Array(dataArray));

	var major = dataArray[3];
	var minor = dataArray[4];

	console.log('major:' + evothings.util.toHexString(major) + ', minor:' + evothings.util.toHexString(minor));
	...
	switch (major) {
		case 0x20:
			...
			peripheral.brewometerData.x = dv.getInt16(0, true);
			peripheral.brewometerData.y = dv.getInt16(2, true);
			peripheral.brewometerData.z = dv.getInt16(4, true);

			peripheral.pitch = -Math.atan(-peripheral.brewometerData.y / Math.sqrt(peripheral.brewometerData.x * peripheral.brewometerData.x + peripheral.brewometerData.z * peripheral.brewometerData.z)) * 180 / Math.PI;
			console.log('acc read: ' + peripheral.brewometerData.x + ',' + peripheral.brewometerData.y + ',' + peripheral.brewometerData.z);
